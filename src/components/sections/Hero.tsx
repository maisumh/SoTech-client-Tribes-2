  ...
  return (
    <section className="pt-[calc(70px+2rem)] pb-12 md:pt-[calc(70px+4rem)] md:pb-20 bg-gradient-to-br from-gray-50 to-offwhite">
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        {/* Audience badges */}
        <motion.div
          className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-12 max-w-[800px] mx-auto"
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.6, ease: "easeOut" }}
        >
          {[
            { icon: "🏘️", label: "For Neighbors", href: "/neighbors" },
            { icon: "🤝", label: "For Organizations", href: "/partners" },
            { icon: "👥", label: "For Community Leaders", href: "/partners" },
          ].map((item) => (
            <a
              key={item.label}
              href={item.href}
